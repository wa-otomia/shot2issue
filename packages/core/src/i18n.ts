...
// Ambient declaration so detectLang's runtime-guarded chrome.i18n probe type-checks
// without @types/chrome. The guard (`typeof chrome !== 'undefined'`) keeps it safe on
// non-extension hosts (desktop), where this resolves to the navigator fallback.
declare const chrome: { i18n?: { getUILanguage?: () => string } } | undefined;
...
const MESSAGES: Record<string, Record<string, string>> = {
  en: {
    ...
    fontSize: 'Font size',
    strokeColor: 'Outline',
    strokeColorTitle: 'Outline / halo color (contrast around lines and text)',
    toolCrop: 'Crop',
    toolCropTitle: 'Crop the screenshot',
    cropApply: 'Apply',
    cropApplyTitle: 'Apply crop (Enter)',
    cropCancel: 'Cancel',
    cropCancelTitle: 'Cancel crop (Esc)',
    ...
    pasteImage: 'Paste',
    pasteImageTitle: 'Paste an image from the clipboard (Ctrl+V)',
    pasteNoImage: 'No image on the clipboard',
    pasteFailed: 'Paste failed: {0}',
    pasteAdded: 'Image added from clipboard',
    ...
    accountsHint: 'Credentials for YouTrack / GitLab instances. Workspaces bind to an account.',
    ...
    accountUntitled: 'Untitled account',
    ...
    hotkeyLabel: 'Global capture hotkey',
    hotkeyRecording: 'Press keys…',
    hotkeyNotSet: 'Not set',
    hotkeyReset: 'Reset to default',
    hotkeyReservedSystem: 'That combination is reserved by your system — try another.',
    hotkeyRegisterFailed: 'Could not register that shortcut — try a different combination.',
    hotkeyErrorDetails: 'Show error details',
    ...
    aiTitlePromptDefault: 'You write concise, specific issue titles. Read the report below and return ONLY the title: a single line, no surrounding quotes, no trailing punctuation, at most about 80 characters. Write the title in the SAME language as the description below — detect the language from that text itself, and do not use any other language (not the interface, system, or account-region language).',
    aiComplaintPromptLabel: 'Dictation prompt',
    aiComplaintPromptDefault: "You turn a user's report (typed or dictated) and screenshots into a clear software issue. Write a concise, specific title and a well-structured Markdown body (what happened, where it happened, steps if any, and expected vs. actual). If a screenshot has numbered boxes (circled numbers), refer to them by number in the description, e.g. \"in screenshot 1, box #1 …\". Write BOTH the title and body in the SAME language as the user's report below — detect the language from that text itself, and do not use any other language (not the interface, system, or account-region language). Return ONLY a JSON object with string fields \"title\" and \"body\".",
    aiUsage: 'Usage',
    ...
    aiStop: 'Stop',
    aiStopped: 'Stopped.',
    aiStateRequesting: 'Requesting…',
    aiStateWriting: 'Writing…',
    aiStateDone: 'Done',
    aiStateTranscribing: 'Transcribing…',
    aiReasoningLabel: 'Reasoning',
    aiReasoningOff: 'No reasoning',
    aiReasoningLow: 'Low',
    aiReasoningMedium: 'Medium',
    aiReasoningHigh: 'High',
    aiActionsHeading: 'AI assistant',
    strokeWidth: 'Outline width',
    autoDictate: 'Auto-dictate',
    dictationLangLabel: 'Dictation language',
    dictationAuto: 'Auto-detect',
    ...
    complaintRecordStart: '🎤 Voice input',
    complaintClear: 'Clear',
    complaintRecordStop: '■ Stop & transcribe',
    aiVocabHeading: 'Voice-input dictionary',
    aiVocabHint: 'Terms (names, jargon) sent to the speech recognizer as a hint so it spells them correctly.',
    aiVocabPlaceholder: 'New term, press Enter to add',
    ...
    popupPaste: 'Paste from clipboard',
  ...
  zh: {
    ...
    fontSize: '字号',
    strokeColor: '描边',
    strokeColorTitle: '描边 / 光晕颜色（在线条和文字周围加对比色）',
    toolCrop: '裁剪',
    toolCropTitle: '裁剪截图',
    cropApply: '应用',
    cropApplyTitle: '应用裁剪（Enter）',
    cropCancel: '取消',
    cropCancelTitle: '取消裁剪（Esc）',
    ...
    copyFailed: '复制失败：{0}',
    pasteImage: '粘贴',
    pasteImageTitle: '从剪贴板粘贴图片（Ctrl+V）',
    pasteNoImage: '剪贴板中没有图片',
    pasteFailed: '粘贴失败：{0}',
    pasteAdded: '已从剪贴板添加图片',
    clipboardImage: '剪贴板图片',
    confirmRemove: '再次点击以确认',
    attDelete: '移除这张截图',
    ...
    tabGeneral: '通用',
    accountsHeading: '账号',
    accountsHint: 'YouTrack / GitLab 实例的凭证。工作空间绑定到账号。',
    ...
    hotkeyNotSet: '未设置',
    hotkeyReset: '恢复默认',
    hotkeyReservedSystem: '该组合被系统占用，请换一个。',
    hotkeyRegisterFailed: '无法注册该快捷键，请换一个组合。',
    hotkeyErrorDetails: '显示错误详情',
    ...
    aiTitlePromptDefault: '你负责撰写简洁、具体的 issue 标题。阅读下面的报告，只返回标题本身：一行、不要加引号、结尾不要标点、不超过约 40 个字。务必使用下方描述本身的语言来写标题（从描述文本判断语言），不要使用其它语言（不要用界面、系统或账号所在地区的语言）。',
    aiComplaintPromptLabel: '口述提示词',
    aiComplaintPromptDefault: '你要把用户口述或输入的内容和截图整理成一个清晰的软件 issue。撰写简洁、具体的标题，以及结构良好的 Markdown 正文（发生了什么、在哪里、复现步骤（如有）、期望与实际）。如果某张截图里有编号框（带圈数字），请在描述中结合编号说明，例如「第 1 张里的 1 号框……」。标题和正文都务必使用下方用户输入内容本身的语言（从该文本判断语言），不要使用其它语言（不要用界面、系统或账号所在地区的语言）。只返回一个包含字符串字段 "title" 和 "body" 的 JSON 对象。',
    aiUsage: '用量',
    aiUsage5h: '5 小时窗口',
    aiUsageWeek: '每周',
    ...
    aiStop: '停止',
    aiStopped: '已停止。',
    aiStateRequesting: '请求中…',
    aiStateWriting: '生成中…',
    aiStateDone: '完成',
    aiStateTranscribing: '识别中…',
    aiReasoningLabel: '思考强度',
    aiReasoningOff: '无思考',
    aiReasoningLow: '低',
    aiReasoningMedium: '中',
    aiReasoningHigh: '高',
    aiActionsHeading: 'AI 助手',
    strokeWidth: '描边粗细',
    autoDictate: '自动听写',
    dictationLangLabel: '听写语言',
    dictationAuto: '自动检测',
    ...
    complaintRecordStart: '🎤 语音输入',
    complaintClear: '清空',
    complaintRecordStop: '■ 停止并转写',
    aiVocabHeading: '语音输入词典',
    aiVocabHint: '把这些词（专有名词、术语）作为提示发给语音识别，帮助它正确拼写。',
    aiVocabPlaceholder: '新词，回车添加',
  ...
  ja: {
    ...
    fontSize: '文字サイズ',
    strokeColor: '縁取り',
    strokeColorTitle: '縁取り / 光輪の色（線や文字の周囲に対比色）',
    toolCrop: 'トリミング',
    toolCropTitle: 'スクリーンショットをトリミング',
    cropApply: '適用',
    cropApplyTitle: 'トリミングを適用（Enter）',
    cropCancel: 'キャンセル',
    cropCancelTitle: 'トリミングをキャンセル（Esc）',
    ...
    pasteImage: '貼り付け',
    pasteImageTitle: 'クリップボードから画像を貼り付け（Ctrl+V）',
    pasteNoImage: 'クリップボードに画像がありません',
    pasteFailed: '貼り付けに失敗しました：{0}',
    pasteAdded: 'クリップボードから画像を追加しました',
    clipboardImage: 'クリップボード画像',
    confirmRemove: 'もう一度クリックで確認',
    ...
    accountsHint: 'YouTrack / GitLab インスタンスの認証情報。ワークスペースはアカウントに紐づきます。',
    ...
    hotkeyLabel: 'グローバルキャプチャショートカット',
    hotkeyRecording: 'キーを押してください…',
    hotkeyNotSet: '未設定',
    hotkeyReset: 'デフォルトに戻す',
    hotkeyReservedSystem: 'その組み合わせはシステムで予約されています。別の組み合わせをお試しください。',
    hotkeyRegisterFailed: 'そのショートカットを登録できませんでした。別の組み合わせをお試しください。',
    hotkeyErrorDetails: 'エラーの詳細を表示',
    ...
    aiTitlePromptDefault: 'あなたは簡潔で具体的な issue タイトルを書きます。下記のレポートを読み、タイトルのみを返してください：1 行、引用符なし、末尾の句読点なし、最大 80 文字程度。タイトルは下記の説明文そのものの言語で書いてください（その文章から言語を判断）。他の言語（UI・システム・アカウントの地域の言語）は使わないでください。',
    aiComplaintPromptLabel: '口述プロンプト',
    aiComplaintPromptDefault: 'ユーザーの記述（入力または口述）とスクリーンショットを、明確なソフトウェア issue にまとめます。簡潔で具体的なタイトルと、整理された Markdown 本文（何が・どこで起きたか、再現手順があれば、期待と実際）を書いてください。スクリーンショットに番号付き枠（丸囲み数字）がある場合は、説明で番号を用いて参照してください（例：「1 枚目の枠 #1 は……」）。タイトルと本文の両方を、下記のユーザー記述そのものの言語で書いてください（その文章から言語を判断）。他の言語（UI・システム・アカウントの地域の言語）は使わないでください。文字列フィールド "title" と "body" を持つ JSON オブジェクトのみを返してください。',
    aiUsage: '使用量',
    aiUsage5h: '5 時間枠',
    aiUsageWeek: '週間',
    ...
    aiStop: '停止',
    aiStopped: '停止しました。',
    aiStateRequesting: 'リクエスト中…',
    aiStateWriting: '生成中…',
    aiStateDone: '完了',
    aiStateTranscribing: '認識中…',
    aiReasoningLabel: '思考の強度',
    aiReasoningOff: '思考なし',
    aiReasoningLow: '低',
    aiReasoningMedium: '中',
    aiReasoningHigh: '高',
    aiActionsHeading: 'AI アシスタント',
    strokeWidth: '縁取りの太さ',
    autoDictate: '自動文字起こし',
    dictationLangLabel: '文字起こしの言語',
    dictationAuto: '自動検出',
    ...
    complaintRecordStart: '🎤 音声入力',
    complaintClear: 'クリア',
    complaintRecordStop: '■ 停止して文字起こし',
    aiVocabHeading: '音声入力の辞書',
    aiVocabHint: 'これらの語（固有名詞・専門用語）を音声認識へのヒントとして送り、正しく書き起こせるようにします。',
    aiVocabPlaceholder: '新しい語、Enter で追加',
