// Unit tests for the pure helpers in @shot2issue/core (no platform, no network).
//
// The ai/* helpers moved VERBATIM from the extension's lib/ai.ts, so these assertions are
// transplanted from apps/extension/tests/ai-unit.mjs. The storage pure helpers (id gen,
// migrateAccounts, accountFor, defaults) moved verbatim from lib/storage.ts, so a handful of
// assertions from the youtrack-unit.mjs era's data-model checks are added against them.
//
// Run after `npm run build` (which compiles src/ -> build/):  node tests/core-unit.mjs
// The run.mjs wrapper builds first, then invokes this.
...
// =====================================================================================
// ai/* pure helpers (transplanted from apps/extension/tests/ai-unit.mjs)
// =====================================================================================
...
// completeAuth requires state to be present and equal whenever pending.state is set).
...
// =====================================================================================
// storage/* pure helpers (verbatim from lib/storage.ts: defaults, id gen, migrate, accountFor)
// =====================================================================================

// --- id generators ---
check('storage: makeId is ws_-prefixed and unique', /^ws_/.test(makeId()) && makeId() !== makeId());
check('storage: makeAccountId is acc_-prefixed', /^acc_/.test(makeAccountId()));
check('storage: makeAttachmentId is att_-prefixed', /^att_/.test(makeAttachmentId()));

// --- defaults ---
check('storage: DEFAULT_CONFIG seeds Change/Bug/Feature types', JSON.stringify(DEFAULT_CONFIG.types) === JSON.stringify(['Change', 'Bug', 'Feature']));
check('storage: DEFAULT_CONFIG closeAfterSubmit on, shortcut off', DEFAULT_CONFIG.closeAfterSubmit === true && DEFAULT_CONFIG.shortcutEnabled === false);
check('storage: DEFAULT_EDITOR_PREFS rect tool, red color', DEFAULT_EDITOR_PREFS.tool === 'rect' && DEFAULT_EDITOR_PREFS.color === '#ff3b30');

// --- migrateAccounts: a legacy inline-credential workspace becomes an Account reference ---
const legacy = {
  ...DEFAULT_CONFIG,
  accounts: [],
  workspaces: [{ id: 'w1', kind: 'youtrack', name: 'YT', baseUrl: 'https://yt.example.com', token: 'perm:abc', project: 'PROJ' }],
};
const migrated = migrateAccounts(legacy);
check('migrate: an account is created for the legacy creds', migrated.accounts.length === 1 && migrated.accounts[0].baseUrl === 'https://yt.example.com' && migrated.accounts[0].token === 'perm:abc');
check('migrate: the workspace now references the account, inline creds stripped',
  migrated.workspaces[0].accountId === migrated.accounts[0].id &&
    migrated.workspaces[0].baseUrl === undefined &&
    migrated.workspaces[0].token === undefined &&
    migrated.workspaces[0].project === 'PROJ');
check('migrate: account name defaults to the host', migrated.accounts[0].name === 'yt.example.com');
check('migrate: is idempotent (re-running adds no duplicate account)', migrateAccounts(migrated).accounts.length === 1);
check('migrate: a github workspace (no creds) is untouched',
  (() => {
    const gh = migrateAccounts({ ...DEFAULT_CONFIG, accounts: [], workspaces: [{ id: 'g', kind: 'github', name: 'GH', owner: 'o', repo: 'r' }] });
    return gh.accounts.length === 0 && gh.workspaces[0].owner === 'o';
  })());

// --- accountFor: resolve a workspace's account (or null) ---
check('accountFor: resolves a bound account', accountFor(migrated, migrated.workspaces[0]) === migrated.accounts[0]);
check('accountFor: null for a workspace with no accountId', accountFor(migrated, { id: 'x', kind: 'github', name: '' }) === null);
check('accountFor: null for a dangling accountId', accountFor(migrated, { id: 'x', kind: 'youtrack', name: '', accountId: 'nope' }) === null);
