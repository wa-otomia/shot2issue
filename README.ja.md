...
その後、エディタの「AI タイトル」が、現在の種類、ページタイトル、ページ URL、説明からタイトルを
生成します。生成に使うシステムプロンプトは設定で編集でき、「既定のプロンプトに戻す」ボタンで現在の
インターフェース言語の既定値にリセットできます。
...
- AI アシスタントは接続するまで無効です。「AI タイトル」を使用すると、タイトル、種類、説明、ページ
  URL（スクリーンショットは含みません）が、タイトルを生成するために OpenAI へ送信されます。その
