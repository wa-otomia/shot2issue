...
スクリーンショットから GitHub・GitLab・YouTrack の issue を作成する Chrome 拡張機能（Manifest
...
- 現在のタブのほか、ツールバーのポップアップから**画面全体・特定のウィンドウ・他のアプリ**も撮影可能（`chrome.desktopCapture`）。各キャプチャ元にそれぞれショートカットを割り当てられ、設定するとポップアップに表示されます。
...
- スマート口述：入力するか口述し（口述はサブスクリプションで文字起こし）、AI がその内容と
  スクリーンショットから issue のタイトルと本文を作成します。
- 複数のワークスペースに対応。各ワークスペースは 1 つの送信先（GitHub リポジトリ、GitLab
  プロジェクト、または YouTrack プロジェクト）を対象とします。YouTrack / GitLab の認証情報は
  再利用可能な**アカウント**に保存され、ワークスペース間で共有されます。
...
から開けます。設定は **ワークスペース**・**アカウント**・**AI**・**一般** のタブに分かれています。

- **アカウント** —— YouTrack / GitLab インスタンスの再利用可能な認証情報：表示名・Base URL・
  トークン（YouTrack の永続トークン、または `api` スコープの GitLab パーソナルアクセストークン）。
  同じインスタンス上の複数のワークスペースが 1 つのアカウントを共有します。GitHub はアカウント不要
  （github.com のウェブセッションを使用）。アカウントはローカルに保存され、設定のバックアップに含まれます。
- **ワークスペース** —— 各ワークスペースは 1 つの送信先です。GitHub：表示名・owner（ユーザーまたは
  組織）・リポジトリ名。YouTrack / GitLab：表示名・アカウント（「アカウント」タブで選択）・プロジェクト
  （YouTrack の短縮名/id、または GitLab の数値 id か `group/project` パス）。認証情報をインラインで
  保存していた旧 YouTrack ワークスペースは自動的にアカウントへ移行されます。
...
GitLab にも文書化された REST API があります。アカウントのパーソナルアクセストークン（`PRIVATE-TOKEN`
ヘッダー、`api` スコープ）を使い、各スクリーンショットをプロジェクトにアップロードし
（`POST /api/v4/projects/:id/uploads`）、返ってきた markdown を本文に埋め込んで issue を作成します
（`POST /api/v4/projects/:id/issues`）。プロジェクトは数値 id か URL エンコードした `group/project`
パスで指定し、セルフホスト環境はアカウントの Base URL で動作します。各インスタンスへの初回送信時に
そのオリジンへのアクセス許可を求めます。
...
Codex の OAuth クライアントは `http://localhost:1455` のコールバックのみを登録しており（拡張機能自身の
chromiumapp.org リダイレクトは `authorize_hydra_invalid_request` で拒否されます）、拡張機能は localhost を
待ち受けできません。そこで「ChatGPT でサインイン」は、その localhost リダイレクトで認可ページを開き、

1. **自動** —— 拡張機能がサインインタブを監視し、到達できない
   `http://localhost:1455/auth/callback?code=…` に遷移した瞬間、タブの URL から `?code=` を直接
   読み取ります（手動操作なし）。これには localhost コールバックのホスト権限が必要で、接続時に
   OpenAI のオリジンと一緒に要求されます。
2. **手動（リンクを貼り付け）** —— 併せて表示されるフォールバック：自動取得されない場合は、その
   「localhost に到達できません」のアドレスをコピーして貼り付けると、拡張機能が PKCE のトークン
   交換を完了します。

その後、エディタの「タイトル要約」が、現在の種類、ページタイトル、ページ URL、説明、および
...
**「スマート口述」。**「スマート口述」ボタンはダイアログを開き、説明を**入力**するか**録音して口述**できます
（録音は ChatGPT サブスクリプション `whisper-1` で文字起こしされます）。モデルはそのテキスト・
スクリーンショット・ページのメタデータからタイトルと Markdown 本文を作成し（構造化 JSON 出力）、
スクリーンショット内の**番号付き枠を番号で参照**します。ダイアログの内容は再オープン後も保持され、
繰り返し生成できます。注意：この文字起こしエンドポイントは Codex デスクトップの非公開ルートで、
ChatGPT セッショントークンのみを受け付けます（API key 不可）。そのため口述の文字起こしはベスト
エフォートで変更される可能性があります（入力は常に利用可能）。キャプチャの失敗（画面キャプチャを含む）
はシステム通知で表示されます。タイトル用プロンプトと同様、口述用のシステムプロンプトも設定で
編集でき、それぞれ「既定のプロンプトに戻す」ボタンがあります。
...
アクセス許可を求めます。AI アシスタントも同様に、接続するときに `https://auth.openai.com/*`・
`https://chatgpt.com/*`・`http://localhost:1455/*`（サインインのコールバックを自動読み取りするため）を要求します。

## プライバシー

- 使用するのはスクリーンショット、タイトル、説明、現在のページ URL のみです。コンソール出力、ネット
  ワーク活動、デバイス情報は収集せず、テレメトリも含みません。
- AI アシスタントは接続するまで無効です。「タイトル要約」を使用すると、種類、説明、ページ URL、
  および現在の（注釈付き）スクリーンショットが、タイトルを生成するために OpenAI へ送信されます。
  「スマート口述」ではさらに録音した音声が文字起こしのために送信されます。その
