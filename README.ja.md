...
**音声「クレーム」。**「クレーム」ボタンはマイクから録音し、ChatGPT サブスクリプション（`whisper-1`）
で音声を文字起こしし、文字起こし・スクリーンショット・ページのメタデータからタイトルと Markdown
本文を作成します（構造化 JSON 出力）。注意：この文字起こしエンドポイントは Codex デスクトップの
非公開ルートで、ChatGPT セッショントークンのみを受け付けます（API key 不可）。そのためこの経路は
ベストエフォートで変更される可能性があり、失敗した場合は録音が失われないよう文字起こしを本文に残します。キャプチャの失敗（画面キャプチャを含む）はシステム通知で表示されます。タイトル用プロンプトと同様、クレーム用のシステムプロンプトも設定で編集でき、それぞれ「既定のプロンプトに戻す」ボタンがあります。
