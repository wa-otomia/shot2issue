...
- 语音「抱怨」：录下口述的抱怨，用你的订阅转写，AI 再据此结合截图撰写 issue 的标题与正文。
...
**「抱怨」。**「抱怨」按钮会弹出一个对话框，你可以**打字输入**抱怨，或**点录音口述**（录音会用你的 ChatGPT 订阅 `whisper-1` 转成文字）。随后模型根据这段文本、截图和页面元数据撰写标题与 Markdown 正文（结构化 JSON 输出），并会**结合截图里的编号框**进行描述。对话框内容在多次打开间保留，可重复生成。注意：该转写接口属于 Codex 桌面版、未公开，且仅接受 ChatGPT 会话令牌（不接受 API key），因此口述转写是尽力而为、可能变动（打字始终可用）。截图失败（含屏幕截图）会以系统通知的形式提示。和标题提示词一样，抱怨的系统提示词也可在设置中编辑，各自带有「恢复默认提示词」按钮。
...
- AI 助手在你连接之前处于关闭状态。当你使用「AI 标题」时，类型、描述、页面 URL 以及当前（带标注的）
  截图会被发送给 OpenAI 以生成标题；「抱怨」还会额外发送录制的音频用于转写。其 token 仅保存在
