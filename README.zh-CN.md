...
- **两种截图方式。** 既能从工具栏截当前标签页，也能**从剪贴板粘贴图片**（弹出菜单里的「从剪贴板粘贴」，或在编辑页按 Ctrl/Cmd+V）。截当前标签页可绑定各自的快捷键，绑定后显示在弹出菜单中。
...
| --- | --- |
| `Ctrl/Cmd+Z` | 撤销上一步标注 |
| `Ctrl/Cmd+C` | 把标注后的截图复制到剪贴板 |
| `Ctrl/Cmd+V` | 把剪贴板中的图片作为新附件粘贴进来 |
| `Ctrl/Cmd+Enter` | 提交正在输入的文字（文字工具） |
| `Esc` `Esc` | 关闭编辑页（连按两次） |

你还可以为**截当前标签页**绑定一个全局快捷键：先在「**设置 → 通用**」中启用，再到 `chrome://extensions/shortcuts` 设定按键。
...
| `unlimitedStorage` | 让多图 / 粘贴的截图能够暂存而不触及 `storage.session` 的小配额。暂存的图片在提交后及编辑页关闭时清除。 |
| `scripting` | 向 github.com 标签页注入提交脚本，以创建 issue 并内联上传截图。 |
| `clipboardRead` | 为「从剪贴板粘贴」及编辑页的粘贴功能读取剪贴板中的图片。 |
| `notifications` | 截图失败时以系统通知提示。 |
...
- 仅截取可见区域；不支持整页或滚动截图。
- `chrome://`、Chrome Web Store 等受限页面有截取限制（会作出提示）；可改用从剪贴板粘贴截图。
- 不会自动设置标签（labels）。
