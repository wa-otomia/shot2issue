...
**语音「抱怨」。**「抱怨」按钮会录制麦克风，用你的 ChatGPT 订阅（`whisper-1`）转写音频，再让模型根据转写文本、截图和页面元数据撰写标题与 Markdown 正文（结构化 JSON 输出）。注意：该转写接口属于 Codex 桌面版、未公开，且仅接受 ChatGPT 会话令牌（不接受 API key），因此这条链路是尽力而为、可能变动；若失败，转写文本会保留在正文中以免丢失录音内容。截图失败（含屏幕截图）会以系统通知的形式提示。和标题提示词一样，抱怨的系统提示词也可在设置中编辑，各自带有「恢复默认提示词」按钮。
