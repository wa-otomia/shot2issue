...
const MESSAGES: Record<string, Record<string, string>> = {
  en: {
    ...
    aiComplaintPromptDefault: "You turn a user's spoken complaint and screenshots into a clear software issue. Write a concise, specific title and a well-structured Markdown body (what happened, where it happened, steps if any, and expected vs. actual). Write in the same language as the complaint. Return ONLY a JSON object with string fields \"title\" and \"body\".",
    aiUsage: 'Usage',
  ...
  zh: {
    ...
    aiComplaintPromptDefault: '你要把用户口述的抱怨和截图整理成一个清晰的软件 issue。撰写简洁、具体的标题，以及结构良好的 Markdown 正文（发生了什么、在哪里、复现步骤（如有）、期望与实际）。使用与抱怨相同的语言。只返回一个包含字符串字段 "title" 和 "body" 的 JSON 对象。',
    aiUsage: '用量',
    aiUsage5h: '5 小时窗口',
    aiUsageWeek: '每周',
  ...
  ja: {
    ...
    aiComplaintPromptDefault: 'ユーザーの話したクレームとスクリーンショットを、明確なソフトウェア issue にまとめます。簡潔で具体的なタイトルと、整理された Markdown 本文（何が・どこで起きたか、再現手順があれば、期待と実際）を書いてください。クレームと同じ言語で書いてください。文字列フィールド "title" と "body" を持つ JSON オブジェクトのみを返してください。',
    aiUsage: '使用量',
    aiUsage5h: '5 時間枠',
    aiUsageWeek: '週間',
