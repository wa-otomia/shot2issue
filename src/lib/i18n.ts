...
const MESSAGES: Record<string, Record<string, string>> = {
  en: {
    ...
    aiComplaintPromptLabel: 'Complaint prompt (voice)',
    aiComplaintPromptDefault: "You turn a user's complaint and screenshots into a clear software issue. Write a concise, specific title and a well-structured Markdown body (what happened, where it happened, steps if any, and expected vs. actual). If a screenshot has numbered boxes (circled numbers), refer to them by number in the description, e.g. \"in screenshot 1, box #1 …\". Write in the same language as the complaint. Return ONLY a JSON object with string fields \"title\" and \"body\".",
    aiUsage: 'Usage',
    ...
    // Editor AI button
    aiTitle: 'AI title',
    aiTitleTitle: 'Generate a title from the description using AI',
    ...
    // Voice complaint
    complaint: 'Complaint',
    complaintTitle: 'Describe a complaint (type or dictate); AI writes the title and description',
    complaintModalTitle: 'Complaint',
    complaintPlaceholder: 'Type your complaint, or use the record button to dictate. Numbered boxes in the screenshots can be referenced by number.',
  ...
  zh: {
    ...
    aiComplaintPromptLabel: '抱怨提示词（语音）',
    aiComplaintPromptDefault: '你要把用户的抱怨和截图整理成一个清晰的软件 issue。撰写简洁、具体的标题，以及结构良好的 Markdown 正文（发生了什么、在哪里、复现步骤（如有）、期望与实际）。如果某张截图里有编号框（带圈数字），请在描述中结合编号说明，例如「第 1 张里的 1 号框……」。使用与抱怨相同的语言。只返回一个包含字符串字段 "title" 和 "body" 的 JSON 对象。',
    aiUsage: '用量',
    aiUsage5h: '5 小时窗口',
    aiUsageWeek: '每周',
    ...
    // 语音抱怨
    complaint: '抱怨',
    complaintTitle: '描述你的抱怨（可打字或语音），AI 据此生成标题和正文',
    complaintModalTitle: '抱怨',
    complaintPlaceholder: '输入你的抱怨，或点录音按钮口述。截图里的编号框可以按编号引用。',
  ...
  ja: {
    ...
    aiComplaintPromptLabel: 'クレーム用プロンプト（音声）',
    aiComplaintPromptDefault: 'ユーザーのクレームとスクリーンショットを、明確なソフトウェア issue にまとめます。簡潔で具体的なタイトルと、整理された Markdown 本文（何が・どこで起きたか、再現手順があれば、期待と実際）を書いてください。スクリーンショットに番号付き枠（丸囲み数字）がある場合は、説明で番号を用いて参照してください（例：「1 枚目の枠 #1 は……」）。クレームと同じ言語で書いてください。文字列フィールド "title" と "body" を持つ JSON オブジェクトのみを返してください。',
    aiUsage: '使用量',
    aiUsage5h: '5 時間枠',
    aiUsageWeek: '週間',
    ...
    // 音声クレーム
    complaint: 'クレーム',
    complaintTitle: 'クレームを記述（入力または音声）。AI がタイトルと本文を作成します',
    complaintModalTitle: 'クレーム',
    complaintPlaceholder: 'クレームを入力するか、録音ボタンで口述してください。スクリーンショット内の番号付き枠は番号で参照できます。',
